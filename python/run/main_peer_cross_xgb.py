...
for k in all_neurons:
	combination[k] = {	'features'	: [i for i in all_neurons if i.split(".")[0] != k.split(".")[0]],
						'targets'	: k,
						'figure'	: ''
						}
#####################################################################
...
relation = {}
for k in ['Pos', 'ADn']:
	key = [i for i in thresholds.iterkeys() if i.split(".")[0] == k] #targets
	fea = [i for i in thresholds.iterkeys() if i.split(".")[0] != k] #features
	tmp = np.zeros((len(key), len(fea)))
	for l in key:
...
for k, i in zip(order, xrange(1,18,3)):
	...
	xticks(np.arange(len(count)), name, rotation = 60)
	simpleaxis(gca())
...
savefig('../../figures/17_XGB_cross_peer_prediction.pdf', dpi = 900, bbox_inches = 'tight')
...
savefig('../../figures/18_XGB_cross_peer_prediction.pdf', dpi = 900, bbox_inches = 'tight')
