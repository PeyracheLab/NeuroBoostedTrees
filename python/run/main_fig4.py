...
combination = {'peer':{}, 'cross':{}}
for g in combination.iterkeys():
	targets = [i for i in list(data) if i.split(".")[0] in ['Pos', 'ADn']]
	for k in targets:
		combination['peer'][k] = {	'features'	: [i for i in targets if i.split(".")[0] == k.split(".")[0] and i != k],
									'targets'	: k
								}
		combination['cross'][k] = { 'features'	: [i for i in targets if i.split(".")[0] != k.split(".")[0]],
									'targets'	: k 
								}
sys.exit()
#####################################################################
